...
def test_daily_quiz_endpoint_retired(client, db, seed_org):
    """Q 통합 3단계-c: GET /students/me/daily-quiz 은퇴(404) + DailyQuizStatus 신규 행이
    더 이상 생기지 않는다(대시보드가 매일 6행을 만들어 오던 마지막 쓰기 경로 제거)."""
    from app.models import DailyQuizStatus

    token = _student_token(client, seed_org)
    assert client.get("/api/v1/students/me/daily-quiz", headers=auth(token)).status_code == 404
    # 대시보드를 조회해도 퀴즈 행이 생기지 않는다(기존 데이터 보존·신규 쓰기만 중단)
    assert client.get("/api/v1/students/me/dashboard", headers=auth(token)).status_code == 200
    assert (
        db.query(DailyQuizStatus)
        .filter(DailyQuizStatus.student_id == seed_org["student"].id)
        .count()
        == 0
    )
...
def test_dashboard_today_is_q_axis(client, db, seed_org):
    """홈 대시보드 '오늘'(Q 통합 3단계-c) — 일일 목표(Q_DAILY_GOAL=10) 진행으로 재정의.
    Q축(chapter_no 있음·서버 채점)만 세고, 퀴즈 축(chapter_no NULL) 시도는 안 잡힌다.
    과목 카드는 todo/progress만 — '과목당 완료' 개념은 퀴즈와 함께 은퇴."""
    token = _student_token(client, seed_org)

    # 퀴즈 축(chapter_no 없음) 5건 — Q 목표에 안 잡혀야 한다
    q = _single_q("국어")
    for i in range(5):
        _game_answer(client, token, "국어", correct=False, last=(i == 4), q=q)
    dash0 = client.get("/api/v1/students/me/dashboard", headers=auth(token)).json()
    assert dash0["today"] == {"done": 0, "total": 10}

    # Q축(chapter_no=2) 3건 → 목표 진행 3/10, 과목 카드 progress
    mq = _single_q("수학")
    for _ in range(3):
        r = client.post(
            "/api/v1/students/me/game-answer",
            json={"subject": "수학", "question_id": mq["id"],
                  "option_id": str(mq["answer"]), "chapter_no": 2},
            headers=auth(token),
        )
        assert r.status_code == 200
    dash = client.get("/api/v1/students/me/dashboard", headers=auth(token)).json()
    assert dash["today"] == {"done": 3, "total": 10}
    mat = next(s for s in dash["subjects"] if s["subject"] == "수학")
    assert mat["state"] == "progress" and mat["done"] >= 1
    assert all(s["state"] in ("todo", "progress") for s in dash["subjects"])
...
def test_chapter_stats_two_axis(client, db, seed_org):
    ...
    # (Q 통합 3단계-c) 퀴즈 분리 노출 필드는 은퇴 — 키 자체가 없다
    assert "daily_quiz_accuracy" not in mat
    # 미학습 챕터는 accuracy=null (0%로 표시하지 않음)
    assert any(c["accuracy"] is None and c["total"] == 0 for c in mat["chapters"])


def test_habit_stats_q_axis(client, db, seed_org):
    """습관 추세(Q 통합 3단계-c) — Q축(서버 채점·chapter_no 있음)만 센다. 자기신고·퀴즈 축은
    집계 제외, 일일 목표(10문항) 달성 시 goal_met=True + 연속 학습일 반영."""
    token = _student_token(client, seed_org)
    # 자기신고 5건(구 퀴즈 축·graded=False) — Q축 아님 → 집계 0 유지
    for _ in range(5):
        client.post("/api/v1/learning/attempts",
                    json={"subject": "국어", "result": "correct", "score": 0, "daily": True},
                    headers=auth(token))
    res0 = client.get("/api/v1/students/me/habit-stats?weeks=1", headers=auth(token)).json()
    assert res0["days"][-1]["attempts"] == 0 and res0["streak"] == 0

    # Q축 서버 채점 10건(정답) → 오늘 목표 달성 — goal_met·streak·정답률 100
    q = _single_q("수학")
    for _ in range(10):
        r = client.post(
            "/api/v1/students/me/game-answer",
            json={"subject": "수학", "question_id": q["id"],
                  "option_id": str(q["answer"]), "chapter_no": 2},
            headers=auth(token),
        )
        assert r.status_code == 200
    res = client.get("/api/v1/students/me/habit-stats?weeks=1", headers=auth(token)).json()
    last = res["days"][-1]  # 오늘
    assert last["attempts"] >= 10 and last["goal_met"] is True and last["accuracy"] == 100
    assert res["streak"] >= 1
...
def test_all_subjects_sticker_retired(client, db, seed_org):
    ...
    # (3단계-c) 스티커·퀴즈 보너스는 값 0 고정을 넘어 응답 키 자체가 은퇴됐다
    assert "sticker_awarded" not in last_res and "quiz_bonus" not in last_res
    assert last_res.get("coins_earned", 0) == 0

    db.expire_all()
    assert db.get(StudentProfile, seed_org["student"].id).coins == before  # 잔액 불변

    # 결과 API의 퀴즈 지도(오늘 완료 과목·다음 과목)·스티커 필드도 키째 은퇴
    res = client.get("/api/v1/students/me/result?subject=국어", headers=auth(token)).json()
    assert "sticker_today" not in res and "today_done" not in res and "subject_order" not in res
