...
DEFAULT_GEN_RULES = (
    ...
    # ★확인 문항(시청 검증) 수율을 높이는 핵심 지침(2026-07-22): 강의를 봐야만 알 수 있는
    # '구체적' 내용을 물어야 자기검증에서 captcha(확인 문항 적합)로 분류된다. 일반 상식·교과서
    # 개념은 블라인드 봇이 그냥 풀어 bank(은행)로 빠지므로 시청 검증에 못 쓴다.
    "- ★그 강의에서 '실제로 말한 구체적인 것'을 물어라 — 강사가 든 예시·수치·순서·정의·"
    "특정 용어·구체적 사례처럼, 그 강의를 봐야만 알 수 있는 내용. 일반 상식이나 교과서 개념처럼 "
    "강의를 안 봐도 풀리는 문제는 피하라(그런 문제는 시청 검증에 쓸 수 없다).\n"
    "- 정답이 자막의 특정 대목에 근거하도록 만들어라(그 대목을 안 보면 못 고르게).\n"
    "- 각 문제는 보기 4개, 정답은 하나.\n"
    "- explain에 정답 해설 1~2문장(가능하면 강의의 어느 내용에 근거하는지 밝혀라).\n"
)
