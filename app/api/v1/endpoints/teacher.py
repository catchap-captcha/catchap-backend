...
def profile(principal: Principal = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    if org and org.code_expires_at:
        code_remain = max(0, (org.code_expires_at - datetime.now()).days)
    return {
        ...
        # (여기에 +9h 보정을 되살리면 test_org_code_expiry_roundtrips_in_kst가 잡는다)
        "code_expires_at": (
            org.code_expires_at.isoformat() if org and org.code_expires_at else None
        ),
