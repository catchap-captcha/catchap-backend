...
EXPECTED_SERVERS: list[tuple[str, str]] = [
    ...
    ("vm-jump", "점프서버 2a"),
    ("vm-ops", "빌드·운영 VM 2a"),
    # ★2026-08-15: 2-b 짝과 NAT 이 늘었다. 그전에는 세 대(점프 2b·운영 2a·운영 2b)가
    #   ★같은 server_key("vm-ops")로 보내고 있었다 — 0814 에 2-b 를 만들며 에이전트 설정을
    #   그대로 복사한 탓이다. 같은 키는 최신값으로 덮어쓰므로 카드 하나가 30초마다
    #   ★서로 다른 서버 값으로 튀었고, 점프 2b 가 "빌드·운영 VM"으로 보였다.
    #   ★키가 곧 카드다 — 서버를 늘리면 키부터 나눠야 한다.
    ("vm-jump-2b", "점프서버 2b"),
    ("vm-ops-2b", "빌드·운영 VM 2b"),
    # ★NAT 은 사설망의 유일한 바깥 출구다. 멈추면 그 영역 파드가 이미지도 못 받는데
    #   그동안 아무 데서도 안 보고 있었다(0815 에 에이전트를 넣었다).
    #   ⚠️키·이름을 "NAT 2a" 로 하면 안 된다 — RETIRED_SERVERS 의 옛 NAT 과
    #   정규화 비교에서 같아져 ★화면에서 감춰진다(실제로 그렇게 만들었다가 고쳤다).
    ("vm-nat-2a", "NAT 출구 2a"),
    ("vm-nat-2b", "NAT 출구 2b"),
]
