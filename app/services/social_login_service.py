    ...
    *,
    ...
    if profile.email:
        ...
        if console_user:
            ...
            # ★detail 을 객체로 주는 이유: 프런트가 이 경우에만 '연결하러 가기' 버튼을 띄워야
            # 하는데, 문구를 문자열로 비교하게 하면 문구를 다듬는 순간 버튼이 조용히 사라진다.
            # 다른 400과 구분되는 기계 판독 값(action)을 함께 준다.
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                detail={
                    "message": (
                        "이 이메일은 콘솔 계정으로 사용 중이에요. 콘솔에 로그인한 뒤 "
                        "'간편 로그인 연결'에서 연결하면 다음부터 소셜 로그인을 쓸 수 있어요."
                    ),
                    "action": "console_link",
                },
            )
