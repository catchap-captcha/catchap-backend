...
`catchap-backend` 서비스 계정이 이 목적을 위해 존재한다 — `Secrets Manager 매니저` +
`KMS 사용자` + `Object Storage 매니저` 조합이고 kms11 접근 명단에 이미 들어 있다.
★역할은 계정을 만든 뒤에 바꿀 수 없어서, 이 계정을 재생성하지 않고 그대로 살려 뒀다.
