...
학생이 카카오·네이버·구글 계정으로 가입·로그인한다. **학생 계정 전용**이다 —
운영자·강사(콘솔 계정)는 종전대로 `/auth/ops-login`만 쓴다. 고권한 계정을 외부 IdP라는
공격면에 두지 않는다는 기존 방침(운영자 로그인 분리, 2026-07-20)과 같은 이유다.
...
| 이메일이 콘솔 계정(users)과 일치 | **400** — 소셜 로그인 대상 아님 |
| 그 외 | `signup_required` |
...
| GET | `/api/v1/auth/social/connections` | 학생 | 내 연결 목록 + has_password |
| POST | `/api/v1/auth/social/{provider}/connect` | 학생 | 로그인 상태에서 추가 연결 |
| DELETE | `/api/v1/auth/social/{provider}` | 학생 | 연결 해제 |
